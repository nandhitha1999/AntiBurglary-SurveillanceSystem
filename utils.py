...
def tf_head(feats, anchors, num_classes, input_shape, calc_loss=False):
...
def tf_correct_boxes(box_xy, box_wh, input_shape, image_shape):
...
def tf_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape):

    box_xy, box_wh, box_confidence, box_class_probs = tf_head(feats,
        anchors, num_classes, input_shape)
    boxes = tf_correct_boxes(box_xy, box_wh, input_shape, image_shape)
...
def tf_eval(tf_outputs,
    ...
    num_layers = len(tf_outputs)
    anchor_mask = [[6,7,8], [3,4,5], [0,1,2]] if num_layers==3 else [[3,4,5], [1,2,3]]
    input_shape = K.shape(tf_outputs[0])[1:3] * 32
    boxes = []
    box_scores = []
    for l in range(num_layers):
        _boxes, _box_scores = tf_boxes_and_scores(tf_outputs[l],
...
def correct_tf_boxes(boxes, image_h, image_w, net_h, net_w):
...
def retrieve_tf_detections(tf_result, anchors, min_probability, nms_thresh, image_input_size, image_size, labels_dict ):

    boxes = []

    for i in range(len(tf_result)):
        # decode the output of the network
        boxes += decode_netout(tf_result[i][0], 
        anchors[i], 
        min_probability, 
        nms_thresh, 
        image_input_size[0], 
    ...
    correct_tf_boxes(boxes, image_size[1], image_size[0], image_input_size[0], image_input_size[1])
