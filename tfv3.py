...
class tfLayer(Layer):
    ...
    def build(self, input_shape):
        super(tfLayer, self).build(input_shape)  # Be sure to call this somewhere!
...
def tfv3_base(input, num_anchors, num_classes):
...
def tfv3_main(input, num_anchors, num_classes):

    input, network_1, network_2, network_3 = tfv3_base(input, num_anchors, num_classes)

    return Model(input, [network_1, network_2, network_3])


def tfv3_train(num_classes,
                anchors,
                max_box_per_image, 
                max_grid, 
                batch_size, 
    ...
    true_tf_1 = Input(shape=(None, None, len(anchors)//6, 4+1+num_classes)) # grid_h, grid_w, nb_anchor, 5+nb_class
    true_tf_2 = Input(shape=(None, None, len(anchors)//6, 4+1+num_classes)) # grid_h, grid_w, nb_anchor, 5+nb_class
    true_tf_3 = Input(shape=(None, None, len(anchors)//6, 4+1+num_classes)) # grid_h, grid_w, nb_anchor, 5+nb_class
    
    
    
    _ , network_1, network_2, network_3 = tfv3_base(input_image, len(anchors)//6, num_classes)
    
    loss_tf_1 = tfLayer(anchors[12:], 
                            ...
                            noobj_scale,
                            xywh_scale,
                            class_scale)([input_image, network_1, true_tf_1, true_boxes])

    loss_tf_2 = tfLayer(anchors[6:12], 
                            ...
                            noobj_scale,
                            xywh_scale,
                            class_scale)([input_image, network_2, true_tf_2, true_boxes])

    loss_tf_3 = tfLayer(anchors[:6], 
                            ...
                            noobj_scale,
                            xywh_scale,
                            class_scale)([input_image, network_3, true_tf_3, true_boxes]) 

    train_model = Model([input_image, true_boxes, true_tf_1, true_tf_2, true_tf_3], [loss_tf_1, loss_tf_2, loss_tf_3])
    infer_model = Model(input_image, [network_1, network_2, network_3])

    return [train_model, infer_model]


def tiny_tfv3_main(input, num_anchors, num_classes):
